...
public interface ConfirmPayment {
     ...
     */
    default void confirmPayment(boolean isMobileNumberEmpty, boolean isPinEmpty,
        JFrame paymentFrame, JTextField number, JPasswordField password) {
    ...
    default boolean inputValidation(JTextField number, JPasswordField password) {
        ...
        if (!n && !p) {
            JOptionPane.showMessageDialog(null,
                "Phone number and Pin number cannot contain character", "Input error",
                JOptionPane.WARNING_MESSAGE);
            return false;
        } else if (!n) {
            JOptionPane.showMessageDialog(null, "Phone Number cannot contain character",
                "Phone number error", JOptionPane.WARNING_MESSAGE);
            return false;
        } else if (!p) {
            JOptionPane.showMessageDialog(null, "Pin number cannot contain character",
